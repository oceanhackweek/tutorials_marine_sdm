...
In this notebook, we will explore two approaches to create background samples, which are needed for our choice of species distribution model algorithm.  
  
These two approaches are as follows:  
- Get occurrences for marine species from OBIS using the `robis` package, and use this as background samples.  
- Select random locations using gridded environmental data as a base for our sampling.  
In both cases, points will be constrain to fit within our area of interest.  
...
# Approach 2. Random points as samples
...
We will use bathymetry data, so the `MS_bathy_5m` layer suits our needs. We will load this data and crop it using our bounding box.  
...
The next notebook will show you how to get environmental data that we will use as inputs in our SDM from the `sdmpredictors` package.  
